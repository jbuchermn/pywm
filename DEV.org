...
- [ ] _pywm_view_update: Don't send general info like pid and parent all the time
- [ ] Downstream floating state (probably don't need upstream) --> set_tiled()
- [ ] Generalized upstream hints (now size_contraints --> size_contraints and layer shell) as array of ints e.g.
